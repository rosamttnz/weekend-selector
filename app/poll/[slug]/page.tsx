...
type Poll = { title: string; slug: string; weekends: string[] };
type Result = { weekend: string; available: number; total: number; unavailable: string[] };
...
export default function PollPage() {
  ...
  if (!poll) return <main className="container">Cargando...</main>;
  ...
  return (
    <main className="container">
      ...
      <p className="description">
        Marca únicamente los fines de semana en los que <strong>NO puedes</strong>.
      </p>
      ...
      {savedToken ? (
        <section className="success">
          ...
          <small>{window.location.origin}/edit/{savedToken}</small>
        </section>
      ) : (
        <>
        ...
        {results.slice(0, 10).map((r, i) => (
          <div className="result" key={r.weekend}>
            <div className="result-main">
              <strong>{i < 3 ? ["🥇", "🥈", "🥉"][i] : `${i + 1}.`} {formatDate(r.weekend)}</strong>
              <small>{r.available}/{r.total} personas disponibles</small>
            </div>
            <span className="result-percent">{r.total ? Math.round(r.available / r.total * 100) : 0}%</span>
          </div>
        ))}
      </section>
    </main>
